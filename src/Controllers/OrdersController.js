...
class OrdersController {
  async create(request, response) {
    ...
    if (!status || !notes) {
      throw new AppError("Status e notas são obrigatórios.", 400);
    }

    const [id] = await knex("orders").insert({
      status : status ?? 1,
      user_id,
      order_total: 0, // Considerar calcular isso com base nos pratos
      notes,
    });
  ...
  async update(request, response) {
    ...
    if (!hasUpdates) {
      return response.status(200).json({ message: "Nenhum dado foi enviado para atualização." });
    }
